...
A aplicação só publica uma resposta automática quando há evidência recuperada da base, fontes citadas e confiança acima do limiar configurado. Quando a evidência é insuficiente, a pergunta é encaminhada para atendimento humano; o rascunho calculado pelo modelo fica disponível no painel administrativo para referência. Respostas humanas validadas têm prioridade quando a nova pergunta corresponde exatamente à pergunta ensinada ou quando apresenta formulação equivalente com os mesmos termos centrais, após normalização segura de caixa, acentos, pontuação, espaços e construções interrogativas genéricas. A equivalência exige cobertura lexical alta e, em caso de empate entre memórias com respostas diferentes, não é aplicada. Nesse caso, a resposta aprovada é reutilizada diretamente, com confiança total e sem solicitar uma nova decisão probabilística ao modelo, evitando que uma reformulação segura volte desnecessariamente à fila humana.
