...
final class SecretBox
{
    ...
    public static function encrypt(string $value, string $appSecret): string
    {
        ...
        if ($ciphertext === false) {
            throw new RuntimeException('Não foi possível proteger a credencial da fonte externa.');
        }
    ...
    public static function decrypt(string $value, string $appSecret): string
    {
        ...
        if ($decoded === false || strlen($decoded) < 28) {
            throw new RuntimeException('A credencial protegida da fonte externa está inválida.');
        }
        $key = self::key($appSecret);
        ...
        if ($plaintext === false) {
            throw new RuntimeException('Não foi possível descriptografar a credencial da fonte externa. Verifique o APP_SECRET.');
        }
        return $plaintext;
    }
    ...
    private static function key(string $appSecret): string
    {
        if (strlen($appSecret) < 32) {
            throw new RuntimeException('APP_SECRET deve ter pelo menos 32 caracteres para proteger credenciais externas.');
        }
