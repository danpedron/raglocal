...
APP_ROOT="${RAG_APP_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}"
ENV_FILE="$APP_ROOT/config/.env"
BACKUP_ROOT="${RAG_BACKUP_ROOT:-/var/backups/rag-app}"
BACKUP_PREFIX="${RAG_BACKUP_PREFIX:-rag-app}"
LOCK_FILE="/run/lock/${BACKUP_PREFIX}.lock"
...
rm -f "$SQL_FILE"
find "$BACKUP_ROOT" -type f -name "${BACKUP_PREFIX}-*.tar.gz" -mtime "+$RETENTION_DAYS" -delete
chmod 600 "$BACKUP_ROOT"/"${BACKUP_PREFIX}"-*.tar.gz
